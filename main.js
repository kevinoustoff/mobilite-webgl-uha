    ...
    map.on('style.load', async() => {
       ...
       await OSMReader.getOverPassData();
        ...
        setInterval( async ()=>{
            ...
            randomPoint = generateRandomCoordinate(sense[selectedSense],center[1], center[0]);
            ...
            // var fullWayCoordinates = await OSMReader.getWayFullData(nearPoint.nodes)
            // var move = false; 
            // for(var i = 0; i<fullWayCoordinates.length;i++){
            
            //    if(fullWayCoordinates[i].type == "node"){
            //          console.log(fullWayCoordinates[i].type)
            //          var from = turf.point(center)
            //          var to = turf.point([fullWayCoordinates[i].lon, fullWayCoordinates[i].lat])
                    
            //          console.log(turf.distance(from,to))
            //          if(turf.distance(from,to) < 0.005){
            //              move = true
            //          }
    
            //         if(move == true){
            //             console.log("giu")
                        if(center[0] == nearcoordinates[0] && center[1] == nearcoordinates[1] ){
                            decisionCarrefour++
